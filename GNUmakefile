...
libaflbluetooth.so: src/bluetooth/BTFuzz.cpp src/bluetooth/Parse.cpp src/bluetooth/Operation.cpp  src/bluetooth/StackAnalyze.cpp src/bluetooth/cJSON.c src/bluetooth/Util.cpp
	$(CXX) -fPIC -Iinclude -I$(LLVM_INCLUDE_DIR) -I$(LLVM_BIN_INCLUDE_DIR) -L$(LLVM_LIB_DIR) -shared -O0 -g $^ -o $@  $(LLVM_LIBS) -ltinfo

btdump: src/bluetooth/Dump.cpp src/bluetooth/Operation.cpp src/bluetooth/Util.cpp
	$(CXX) -O0 -g $^ -o $@
...
afl-fuzz: $(COMM_HDR) include/afl-fuzz.h $(AFL_FUZZ_FILES) src/afl-common.o src/afl-sharedmem.o src/afl-forkserver.o src/afl-performance.o libaflbluetooth.so | test_x86
	$(CC) $(CFLAGS) -O0 -g $(COMPILE_STATIC) $(CFLAGS_FLTO) $(AFL_FUZZ_FILES) libaflbluetooth.so src/afl-common.o src/afl-sharedmem.o src/afl-forkserver.o src/afl-performance.o -o $@ $(PYFLAGS) $(LDFLAGS) -lm 
...
.PHONY: clean
clean:
	rm -rf libaflbluetooth.so $(PROGS) afl-fuzz-document afl-as as afl-g++ afl-clang afl-clang++ *.o src/*.o *~ a.out core core.[1-9][0-9]* *.stackdump .test .test1 .test2 test-instr .test-instr0 .test-instr1 afl-cs-proxy afl-qemu-trace afl-gcc-fast afl-g++-fast ld *.so *.8 test/unittests/*.o test/unittests/unit_maybe_alloc test/unittests/preallocable .afl-* afl-gcc afl-g++ afl-clang afl-clang++ test/unittests/unit_hash test/unittests/unit_rand *.dSYM lib*.a
...
install: all $(MANPAGES)
	...
	@if [ -f libaflbluetooth.so ]; then set -e; install -m 775 libaflbluetooth.so $${DESTDIR}$(HELPER_PATH); fi
...
uninstall:
	...
	-cd $${DESTDIR}$(HELPER_PATH) && rm -f afl-g*.*o afl-llvm-*.*o afl-compiler-*.*o libaflbluetooth.so libdislocator.so libtokencap.so libcompcov.so libqasan.so afl-frida-trace.so libnyx.so socketfuzz*.so argvfuzz*.so libAFLDriver.a libAFLQemuDriver.a as afl-as SanitizerCoverage*.so compare-transform-pass.so cmplog-*-pass.so split-*-pass.so dynamic_list.txt
