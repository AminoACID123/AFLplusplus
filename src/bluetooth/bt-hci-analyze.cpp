...
bool HCIVisitor::TraverseCXXRecordDecl(CXXRecordDecl *decl)
{
    ...
    if (name.find("bt_hci_cmd") == name.npos && name.find("bt_hci_evt") == name.npos)
        return true;
    ...
    else if (name.find("bt_hci_evt_le") != name.npos)
    {
        ...
        hci_le_evts[opcode].size = get_size(decl);
    }
    else if (name.find("bt_hci_evt") != name.npos)
    {
        opcode = name_to_hci_evt[name.upper()];
        hci_evts[opcode].size = get_size(decl);
    }
    else if (name.find("bt_hci_rsp") != name.npos)
    {
        ...
        hci_cmds[opcode].rsp_size = get_size(decl);
    }

    return true;
}

int HCIVisitor::get_size(CXXRecordDecl *decl)
{
    int size = 0;
    for (FieldDecl *FD : decl->fields())
    {
        ...
        {
            size += get_size(T->getAsStructureType()->getAsCXXRecordDecl());
        }
        ...
        {
            int tmp;
            sscanf(Tname.c_str(), "uint%d_t", tmp);
            size += tmp / 8;
        }
        else
        {
...
void HCIMacroCallback::MacroDefined(const Token &MacroNameTok, const MacroDirective *MD)
{
    std::string macro = MacroNameTok.getIdentifierInfo()->getName().str();
    if (macro.find("BT_HCI_CMD") == macro.npos && macro.find("BT_HCI_EVT") == macro.npos)
        return;
    ...
    if (macro.find("BT_HCI_CMD") != macro.npos)
    {
        ...
        hci_cmds[opcode].opcode = opcode;
        name_to_hci_cmd[macro] = opcode;
    }
    else if (macro.find("BT_HCI_EVT_LE") != macro.npos)
    {
        hci_le_evts[opcode].name = macro;
        hci_le_evts[opcode].opcode = opcode;
        name_to_hci_le_evt[macro] = opcode;
    }
    else if (macro.find("BT_EVT") != macro.npos)
    {
        hci_evts[opcode].name = macro;
        hci_evts[opcode].opcode = opcode;
        name_to_hci_evt[macro] = opcode;
    }
}

std::unique_ptr<clang::ASTConsumer> ExtractHCIAction::CreateASTConsumer(clang::CompilerInstance &Compiler,
                                                                        llvm::StringRef InFile)
{
    std::unique_ptr<HCIMacroCallback> C(new HCIMacroCallback(&Compiler.getSourceManager()));
...
{
    auto ExpectedParser =
        clang::tooling::CommonOptionsParser::create(argc, argv, MyToolCategory, llvm::cl::NumOccurrencesFlag::Required);
    if (!ExpectedParser)
    {
