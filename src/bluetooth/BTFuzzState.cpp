...
// random operations
// random events
u32 BTFuzzState::fuzz_one_sema(u8 *buf)
{
    u32 rand1 = rand_below(100);
    if (rand1 < 80 && !pcmd.empty())
    {
        rand1 % pcmd.size();
    }

    u32 rand2 = rand_below(100);
    if (rand2 < 5)
    {
    ...
    if (rand2 < 10)
    {
        Operation *op = get_operation(CORE_OPERATION_GAP_CONNECT_CANCEL);
    }
    if (rand2 < 15)
    {
    ...
    return ((item_t *)buf)->size + sizeof(u32);
}

u32 BTFuzzState::fuzz_one_rand(u8 *buf)
{
...
void BTFuzzState::handle_evt(hci_event_t *evt)
{
    ...
    case BT_HCI_EVT_CMD_COMPLETE: {
        ...
        for (auto it = SET(pcmd).begin(), eit = SET(pcmd).end(); it != eit; ++it)
        {
    ...
    case BT_HCI_EVT_CMD_STATUS: {
        ...
        for (auto it = sPending_cmd.begin(), eit = sPending_cmd.end(); it != eit; ++it)
        {
            hci_command_t *cmd = (hci_command_t *)it->data();
            if (e->opcode == cmd->opcode)
            {
                if (e->opcode == BT_HCI_CMD_CREATE_CONN && e->status == BT_HCI_ERR_SUCCESS)
                {
                    hci_con con;
                    bt_hci_cmd_create_conn *c = (bt_hci_cmd_create_conn *)cmd->param;
                ...
                else if (e->opcode == BT_HCI_CMD_DISCONNECT && e->status == BT_HCI_ERR_SUCCESS)
                {
                    bt_hci_cmd_disconnect *c = (bt_hci_cmd_disconnect *)cmd->param;
    ...
    case BT_HCI_EVT_DISCONNECT_COMPLETE: {
        bt_hci_evt_disconnect_complete *e = (bt_hci_evt_disconnect_complete *)evt->param;
        if (e->status == BT_HCI_ERR_SUCCESS)
        {
    ...
    case BT_HCI_EVT_LE_META_EVENT: {
        ...
        case BT_HCI_EVT_LE_CONN_COMPLETE: {
            bt_hci_evt_le_conn_complete *e = (bt_hci_evt_le_conn_complete *)&evt->param[1];
            if (e->status == BT_HCI_ERR_SUCCESS)
            {
                hci_con c;
                c.handle = e->handle;
