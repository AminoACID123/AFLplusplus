...
template <typename T>
struct
    is_class_imp { //采用boost的type_traits的方式判断，判断一个类型是否是一个类类型
  ...
  template <typename C>
  static class_type is_class_check(void (C::*)(void)); //类类型匹配到的模板函数

  template <typename C>
  static non_class_type is_class_check(...); //基础类型匹配到的模板函数

  static const bool value = (sizeof(is_class_check<T>(0)) ==
                             sizeof(class_type)); // value的值在编译期决定
};

template <>
struct is_class_imp<
    string> { //模板特化，string可以作为基础类型处理，其实是类类型

  static const bool value = false;
};

template <typename T> struct is_class : is_class_imp<T> {}; //继承

template <bool C_>
struct bool_plt {}; //用于编译期条件判断的模板，bool_plt<true>和bool_plt<false>

template <typename C_, typename F1,
          typename F2> // C_编译期的条件，依据条件判断，动态定义类型F1或F2
struct eval_if {};

template <typename F1, typename F2> //模板偏特化，typename C_

struct eval_if<bool_plt<true>, F1,
               F2> { //当C_编译期条件为bool_plt<true>时，定义类型F1

  typedef F1 type;
};

template <typename F1, typename F2> //模板偏特化，typename C_
struct eval_if<bool_plt<false>, F1,
               F2> { //当C_编译期条件为bool_plt<false>时，定义类型F2
...
struct CFreeInvoke { //序列化和反序列化统一调用模版函数，在编译期决定调用其一
  static void invoke(Archive &ar, T &t) {
    typedef
        typename eval_if<typename Archive::is_marshall, //假如ar对象是序列化对象
                         CFreeMarshall<Archive, T>, //定义序列化类型
                         CFreeDemarshall<Archive, T>>::type
            typex; //否则定义反序列化类型
...
public:
  ...
  serialize(Archive &ar,
            vector<T> &t) //调用序列化或反序列化函数，在编译期动态判断决定
...
public:
  ...
  void init(uint32_t ver, int i, bool b, long l, double d, int arr[], int len)

  {
    ...
    this->vecInt.assign(arr, arr + len);
  }
...

{
  ...
  { //使用模板方式，在编译期判断数据类型，是否是类类型
    ...
    cout << std::is_class<int>::value << endl; //该代码块都是基础数据类型

    cout << std::is_class<double>::value << endl;

    cout << std::is_class<EName>::value << endl;

    cout << std::is_class<string>::value << endl;

    cout << std::is_class<CData>::value << endl; //该代码块都是类类型

    cout << std::is_class<SData>::value << endl;

    cout << std::is_class<vector<int>>::value << endl << endl;
  }
  ...
  { //序列化和反序列化类类型
    ...
    oData.init(0, 11, true, 222, 3.30,  arr, len);
