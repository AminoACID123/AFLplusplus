...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  if (!afl->queue_cur)
    ...
    //       // s32 fd = open(fn, O_WRONLY | O_CREAT | O_TRUNC,
    //       DEFAULT_PERMISSION);
...
bt_fuzz_stage:
  ...
  if (afl->bt) {
    if (bt_sema_enabled()) {
      ...
      afl->stage_short = "BTRand";
      afl->stage_name = "BT Random";
    }
    ...
    if (!afl->queue_cur) {
      bt_deserialize_state(NULL);
      bt_len = 0;
    } else {
      bt_deserialize_state(afl->queue_cur->bt_state_buf);
      items = bt_item_nr(bt_in, len);
      bt_len = len;
    }
    if(items >= BT_MAX_ITEM_COUNT)
      goto abandon_entry;
    ...
    LIST_FOREACH(&afl->custom_mutator_list, struct custom_mutator, {
      if (el->stacked_custom && el->afl_custom_havoc_mutation_probability) {
        ...
        if (el->stacked_custom_prob > 100) {
          ...
          FATAL("The probability returned by "
...
#ifdef INTROSPECTION
          snprintf(afl->m_tmp, sizeof(afl->m_tmp), " SPLICE_INSERT-%u-%u-%u-%s",
    ...
    if (afl->key_puppet == 1) {
      ...
      if (unlikely(afl->orig_hit_cnt_puppet == 0)) {
        ...
        afl->SPLICE_CYCLES_puppet =
            (rand_below(afl, SPLICE_CYCLES_puppet_up -
                                 SPLICE_CYCLES_puppet_low + 1) +
          ...
          case 7:
            ...
            if (rand_below(afl, 2)) {
              ...
              *(u16 *)(out_buf + pos) -= 1 + rand_below(afl, ARITH_MAX);
              ...
              u16 num = 1 + rand_below(afl, ARITH_MAX);
#ifdef INTROSPECTION
              snprintf(afl->m_tmp, sizeof(afl->m_tmp), " ARITH16BE-%u-%u", pos,
                       num);
              strcat(afl->mutation, afl->m_tmp);
#endif
              *(u16 *)(out_buf + pos) =
          ...
          case 8:
            ...
            if (rand_below(afl, 2)) {
              ...
              *(u32 *)(out_buf + pos) -= 1 + rand_below(afl, ARITH_MAX);
              ...
              u32 num = 1 + rand_below(afl, ARITH_MAX);
#ifdef INTROSPECTION
              snprintf(afl->m_tmp, sizeof(afl->m_tmp), " ARITH32BE_-%u-%u", pos,
                       num);
              strcat(afl->mutation, afl->m_tmp);
#endif
              *(u32 *)(out_buf + pos) =
            ...
            if (rand_below(afl, 2)) {
              ...
              *(u32 *)(out_buf + pos) += 1 + rand_below(afl, ARITH_MAX);
              ...
              u32 num = 1 + rand_below(afl, ARITH_MAX);
#ifdef INTROSPECTION
              snprintf(afl->m_tmp, sizeof(afl->m_tmp), " ARITH32BE+-%u-%u", pos,
                       num);
              strcat(afl->mutation, afl->m_tmp);
#endif
              *(u32 *)(out_buf + pos) =
