...
void mutate_parameter(afl_state_t *afl, char *buf, int len,
                      bt_mutator mutator) {
...
void get_parameter(char *buf, int len, int param, char **out_buf,
                   int *out_len) {
  int i = 0;
  int pos = 0;
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    for (i = 0; i < use_stacking; ++i) {
      ...
      switch ((flag = r = rand_below(afl, r_max))) {
        ...
        case 0 ... 3: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_flip_bit);
          break;
        }

        /* Set byte to interesting value. */
        case 4 ... 7: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_interesting8);
          break;
        }

        /* Set word to interesting value, little endian. */
        case 8 ... 9: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_interesting16_le);
          break;
        }

        /* Set word to interesting value, big endian. */
        case 10 ... 11: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_interesting16_be);
          break;
        }

        /* Set dword to interesting value, little endian. */
        case 12 ... 13: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_interesting32_le);
          break;
        }

        /* Set dword to interesting value, big endian. */
        case 14 ... 15: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_interesting32_be);
          break;
        }

        /* Randomly subtract from byte. */
        case 16 ... 19: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_subtract8);
          break;
        }

        /* Randomly add to byte. */
        case 20 ... 23: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_add8);
          break;
        }

        /* Randomly subtract from word, little endian. */
        case 24 ... 25: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_subtract16_le);
          break;
        }

        /* Randomly subtract from word, big endian. */
        case 26 ... 27: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_subtract16_be);
          break;
        }

        /* Randomly add to word, little endian. */
        case 28 ... 29: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_add16_le);
          break;
        }

        /* Randomly add to word, big endian. */
        case 30 ... 31: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_add16_be);
          break;
        }

        /* Randomly subtract from dword, little endian. */
        case 32 ... 33: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_subtract32_le);
          break;
        }

        /* Randomly subtract from dword, big endian. */
        case 34 ... 35: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_subtract32_be);
          break;
        }

        /* Randomly add to dword, little endian. */
        case 36 ... 37: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_add32_le);
          break;
        }

        /* Randomly add to dword, big endian. */
        case 38 ... 39: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_add32_be);
          break;
        }

        /* Just set a random byte to a random value. */
        case 40 ... 43: {
          mutate_parameter(afl, out_buf, temp_len, bt_mutator_random8);
          break;
        }
